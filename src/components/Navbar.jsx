...
function Navbar() {
  ...
  return (
	<nav className='w-full h-[70px] flex justify-between items-center navbar bg-primary'>
		...
		<ul className='hidden sm:flex justify-between items-center list-none px-6'>
      {
        navLinks.map((link, index)=>(
          <li key={link.id} className={`text-white font-bold ${index === navLinks.length-1 ? 'mr-0' : 'mr-10' }`}>
            <a href={`#${link.id}`}>
              {link.title}
            </a>
          </li>
    ...
    <div className='flex sm:hidden justify-start items-center px-6'>
      <img src={toggle ? close : menu} alt='menu' onClick={()=> setToggle((prev)=> !prev)} className='w-[25px] h-[25px] object-contain' />
      <div className={`${toggle ? 'flex' : 'hidden'}`}>
        <ul className={`absolute right-0 bg-primary bg-opacity-70 top-[70px] z-[10]`}>
          {
            navLinks.map((link, index)=> (
