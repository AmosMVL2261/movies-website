...
function UpcomingCard({title, backdrop_path, release_date}){
  return (
    ...
    <img className='rounded-md' src={`https://image.tmdb.org/t/p/w300/${backdrop_path}`} alt="" />
...
function UpcomingMovies() {
  ...
  const getData = async () => {
    ...
    let request = await axios.get(requestLink).then((response) => (response.data.results))
  ...
  return (
    ...
    <div className='flex flex-col justify-center items-center flex-wrap sm:flex-row '>
      {
        data.map((movie)=>(
          <UpcomingCard key={movie.id} title={movie.title} backdrop_path={movie.backdrop_path} release_date={movie.release_date} />
        ))
      }
    </div>
	</div>
