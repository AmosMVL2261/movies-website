...
function MovieCard({ title, vote_average, poster_path }){
  return (
    ...
    {/* <div className='flex flex-col items-center h-[388px] w-[190px] p-2 border rounded-xl bg-white'> */}
      <div className=''>
        <img className='h-[278px] object-fill rounded-md' src={`https://image.tmdb.org/t/p/w185/${poster_path}`} alt="movie" />
      </div>
      <div className='h-[110px] max-w-[185px] flex flex-col justify-between items-center'>
        <h3 className='font-roboto text-center w-[100%] overflow-hidden font-bold text-base text-white pt-2'>{title}</h3>
...
function Genre({title, genreID}) {
  ...
  const getData = async () => {
    ...
    const data = await axios.get(requestLink).then((response) => (response.data.results))
    setMovies(data)
  }
  ...
  return (
    ...
    <div className='flex justify-center items-center h-[400px] w-[100%]'>
      <div className='w-full'>
        <Carousel itemClass="carousel-item-padding-40-px" responsive={responsive} draggable={false} centerMode={true} infinite>
          {
            movies.map((movie) => (
              <MovieCard key={movie.id} title={movie.title} vote_average={movie.vote_average} poster_path={movie.poster_path} />
            ))
          }
        </Carousel>
      </div>
